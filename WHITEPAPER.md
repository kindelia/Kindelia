...
Technically, Kindelia is just a decentralized REPL that interprets statements in
a programming language, and Kindelia blocks are just files in that language.
These blocks are efficiently compressed, propagated through the network via
Nakamoto Consensus, and computed by nodes efficiently and measurably using the
HVM. And that's all. No currency, account, premine or fancy crypto. Just a
peer-to-peer functional REPL around the HVM, as if we made a massively
multiplayer [GHCi](https://downloads.haskell.org/~ghc/latest/docs/html/users_guide/ghci.html).
...
reused store. That cost is high for two reasons: first, to limit the state
growth; second, because a single store demands an expensive Merkle Patricia Tree
insertion. Kindelia treats state differently: it just saves reversible snapshots
of HVM's heap. Because of that, a reused SSTORE has 0 cost! This makes
programming much more convenient, since contracts can store arbitrary HVM
...
stack machines: beta reduction and pattern matching are primitive, O(1) opcodes
on the HVM.
